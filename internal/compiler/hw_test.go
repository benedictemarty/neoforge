...
func TestHwForms(t *testing.T) {
	src := strings.Join([]string{
		...
		`sfx 1,2`, `vmode 1`, `wait 2`, `mouse to 1,2`, `mouse show`, `mouse hide`, `mouse cursor 1`,
		`pin 1, input`, `pin 1, output`, `pin 1, analog`, `pin 1, 0`, `iwrite 1,2,3`,
		`x = pin(1) + analog(2) + havemouse() + iread(1,2) + mouse(mx, my) + mouse(mx, my, mw)`, `ink 3`, `ink 3,1`, `cursor 1,2`, `palette 1,2,3,4`, `palette clear`,
	...
	for _, c := range []struct{ src, want string }{
		...
		{`mouse 1`, "mouse : « to »"},
		{`mouse to 1`, "« , » attendu"},
		{`mouse cursor "a"`, "nombre attendu"},
		{`pin "a"`, "nombre attendu"},
		{`pin 1`, "« , » attendu"},
		{`pin 1, "a"`, "nombre attendu"},
		{`iwrite 1`, "« , » attendu"},
		{`x = mouse(1, 2)`, "par référence"},
		{`x = mouse(mx, "a")`, "nombre attendu"},
